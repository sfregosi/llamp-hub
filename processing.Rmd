...
Create a new post-processing issue for the latest trip, and add it to the [Longline Acoustic Monitoring GitHub Project](https://github.com/users/sfregosi/projects/3).

- Navigate to the [llamp](https://github.com/sfregosi/llamp) repository's [New Issue](https://github.com/sfregosi/llamp/issues/new/choose) page and press the green 'Get Started' button for the **Data processing template**

```{r ss-github, dpi = 80, fig.cap = 'Screenshot showing the new issue chooser with just the Post-processing template shown. Select the \'Data processing template\''}
knitr::include_graphics('images/screenshot_github_post-processing-template.png')
```

**Create issue manually using a 'blank' issue:**

2. Copy the below [text](https://github.com/sfregosi/llamp/blob/main/docs/files/post-processing_checklist.txt) into the body of a [new, blank issue](https://github.com/sfregosi/llamp/issues/new) and it will be formatted and ready to go in markdown syntax. **Name the issue 'LL0## - Data Processing'**

```{verbatim}
**Metadata tasks**
- [ ] fill in relevant deployment info on [LLHARP Data Summary](https://docs.google.com/spreadsheets/d/1S0e3T8GijoM0NTLCLjKV_9d3C9_YnHNqZhkV0BNJkzg) (e.g., frame and data logger numbers, sd card useage, recovery voltages, TDR info)

**HRP imaging tasks**
- [ ] run `dd.exe` stuff on SD card to create .hrp file. Initially created on external HD. *This is a several-hours long process*
- [ ] copy HRP(s) to server. Retain copy of HRP on external hard drive

**HRP processing tasks** (can be done on external hard drive *or* remotely on virtual machine once HRP is uploaded to the server)	
- [ ] create parameter txt file (and save to server 'HRP' folder)
- [ ] set up MATLAB paths for correct Triton versions
- [ ] process HRP into xwavs
- [ ] create LTSAs of xwavs	

**Data upload tasks**
- [ ] put copy of HRP on server
- [ ] save copy of HRP on local physical drive **(and note this somewhere...)**
- [ ] upload xwavs on server (if created locally, can be removed from local HD once they have been checked)
- [ ] upload in-air recordings to server
```
...
```{r ss-partitions, dpi = 60, fig.cap = 'Screenshot showing output for listing the partitions'}
...
```{r ss-img-HRP, layout = 'l-body-outset', fig.cap = 'Screenshot showing what the imaging process should look like after running the dd.exe'}
...
```{r ss-process-HRP, dpi = 80, fig.cap = 'Screenshot showing output as it should be if there are no errors while processing the HRP to xwavs'}
...
[LLHARP Part II - Data
Processing](https://docs.google.com/document/d/1po1G2isgSCfYWc1dj8XphotPeZuOZ22ugL5wsc9o3JY/edit)
