...
Candidate odontocete encounters are first manually logged in MATLAB-based program Triton. This identifies the actual areas of interest for odontocete detection and classification and excludes periods of just glider or ambient noise. 
...
- Clean up and collapse final events into a .csv 
    ...
    - Must include columns 'start', 'stop', 'sp', and 'id' 
...
### Click, whistle, and burst pulse detection

Pamguard is used to detect individual clicks, whistles, and burst pulses for further analysis. For ease and to prepare for future analyses, Pamguard is run on across the full glider survey dataset. 
...
- Save a copy of the preferences file specific for this survey before making any modifications
  ...
  - Name the file the base file name plus the survey string e.g., `pam20207b_llharp_banter_LL060_FR38_DL65.psfx`
...
- Modify the data input to the Sound Acquisition module
  ...
  - Navigate to the `.x.wav` folder that contains all .x.wav files for a single recorder, select the folder and press *Select files and folders*. It is ok if this folder also contains an .ltsa or other non .x.wav files
...
```{r ss-sound-aquisition, dpi = 60, fig.link = 'images/screenshot_sound_acquisition_module_folder.png', fig.cap = 'Screenshot showing the correct configuration the Sound Aquisition module'}
...
Each glider will be run individually and this will take several hours to days to run
...
- Make a copy of both the database and folder of binaries and store on the data hard drive in a folder called `originals` within the `pamguard` folder. 


## Classification

The clicks, burst pulses, and whistles detected using Pamguard are exported and fed into the classification R package `banter` to try to identify which encounters were possibly FKW and which were likely other odontocete species. 

This protocol/methodology is not as well fleshed out yet...  
